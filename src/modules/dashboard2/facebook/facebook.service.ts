...
function getRandomGeneralEngagementMessage(rate: number): string {
  if (rate >= 90) {
    return getRandomMessage(["💥 Phenomenal engagement — your audience is fully invested in your content.", "🌐 You're setting the standard — this level of loyalty signals brand leadership.", "🚀 Peak performance — now is the time to scale ads, collaborations, or premium content."]);
  } else if (rate >= 70) {
    return getRandomMessage(["🔥 Outstanding engagement — you're connecting with depth and consistency.", "🏆 Elite-level response — few achieve this. Consider capturing insights from top posts.", "📣 You're in the spotlight — maintain variety while reinforcing your core strengths."]);
  } else if (rate >= 60) {
    return getRandomMessage(["🎯 Very strong engagement — your creative strategy is clearly working.", "📊 Strong signals — use this traction to test high-value campaigns or audience segments.", "📈 High response rate — consider launching a series or deeper community content."]);
  } else if (rate >= 50) {
    return getRandomMessage(["✅ Strong engagement — you're outperforming industry norms.", "🚧 Above-benchmark performance — keep your current rhythm but test small pivots.", "💡 You're in a growth zone — now is the time to fine-tune frequency and style."]);
  } else if (rate >= 30) {
    return getRandomMessage(["📌 Good engagement — your message is landing with a significant portion of your audience.", "🔍 Consistent visibility — analyze which formats drive this and amplify them.", "🧭 You're on the map — double down on your unique tone or value proposition."]);
  } else if (rate >= 20) {
    return getRandomMessage(["📉 Moderate engagement — content has potential but lacks punch.", "📤 Partial connection — refine storytelling and lean into community cues.", "🛠️ You're warming up — think iterative testing of visuals and message framing."]);
  } else if (rate >= 10) {
    return getRandomMessage(["⚠️ Mediocre engagement — occasional wins, but overall impact is muted.", "🧪 Some traction, but inconsistent — tighten your theme and A/B test frequently.", "📆 Review post timing and content hooks — you’re not far from stronger results."]);
  } else if (rate >= 5) {
    return getRandomMessage(["🔧 Low engagement — your content isn’t sticking. Explore underserved content gaps.", "🕵️‍♂️ You're missing emotional or practical appeal — audit tone and intent.", "📉 Weak response — are you solving a clear problem or sparking curiosity?"]);
  } else if (rate >= 1) {
    return getRandomMessage(["❓ Very low engagement — your content is being ignored or scrolled past.", "🪞 Take a hard look — is your content visually compelling and audience-relevant?", "⏳ Your audience isn't connecting — rethink the story or experience you're offering."]);
  } else {
    return getRandomMessage(["⛔ No engagement — this content strategy isn't working at all.", "💀 You're invisible — shift focus, tone, and value proposition immediately.", "📵 No traction — reboot with audience research, new formats, or even a brand refresh."]);
  }
}
...
function calculateEngagementStats(posts: any[], followerCount: number) {
  ...
  const avgRatio = perPostEngagement.reduce((sum, p) => sum + parseFloat(p.engagementToFollowerRatio), 0) / perPostEngagement.length;
...
export const getFacebookPostsFromScrapedData = async (facebook_handle: string, max_posts: number = 3, retry_attempts: number = 4, delay: number = 2) => {
  const headers = { "x-api-key": API_KEY };
  const handle = facebook_handle.trim();
  const isPageId = /^\d+$/.test(handle);
  const queryParam = isPageId ? `pageId=${handle}` : `url=https://${handle.replace(/^https?:\/\//, "")}`;
  ...
  try {
    while (total_posts < max_posts && empty_post_retries > 0) {
      const url: any = cursor ? `${FACEBOOK_POST_URL}?${queryParam}&cursor=${cursor}` : `${FACEBOOK_POST_URL}?${queryParam}`;
