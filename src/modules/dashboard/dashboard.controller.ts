...
export const getUserDashboard = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const reportPriceMap: Record<string, number> = Object.fromEntries(analysisServices.map((service) => [service.report, service.price]));
    ...
    const dashboardCounts = {
      ...
      total_cmo_recommendations: Array.isArray(finalRecommendationByCMO) ? finalRecommendationByCMO.length : finalRecommendationByCMO ? 1 : 0,
    };
...
export const getWebsiteDetailedAnalysis = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { traffic_analysis, onpage_optimization, technical_seo, ...otherSeoFields } = seoAuditData;
      ...
      userWebsites
        .map(async (site) => {
          ...
          for (const report of site.report) {
            ...
            const hasAnyField = (fields: string[]) => fields.some((field) => report[field as keyof typeof report] != null);
