...
export async function scrapeWebsiteCompetitors(url: string) {
  try {
    ...
    if (allSitemapUrls.length > 7) {
      ...
      const merged = [homepage, ...keywordMatched, ...shallowPages].filter((v, i, self) => self.indexOf(v) === i).slice(0, 7);
...
let userReq = await prisma.user_requirements.findFirst({
  ...
  const { industry, region_of_operation, target_location, target_audience, USP, primary_offering } = userReq;
