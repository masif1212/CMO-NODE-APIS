...
// // try {
// //       html = await fetchFullHtml(website_url); // Puppeteer-based
...
//   // DNS lookup (only if HTML was fetched successfully)
//   const dnsResult = await dns.lookup(domain);
//   ipAddress = dnsResult.address;

// } catch (error: any) {
//   // Puppeteer or fetch failed
//   statusCode = error?.statusCode || error?.response?.status || 500;

//   // Sometimes Puppeteer errors don’t have response body
//   const raw = error?.response?.data || error?.html || "";
//   html = typeof raw === "string" ? raw : "";
